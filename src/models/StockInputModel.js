export default (sequelize, DataType) => {
  ...
  sequelize.models.people.hasMany(StockInput, {
    foreignKey: 'id_people',
    targetKey: 'id_people',
  })
  StockInput.belongsTo(sequelize.models.people, {
    foreignKey: 'id_people',
    targetKey: 'id_people',
  })
  StockInput.getOne = id_stock => {
    return StockInput.findAll({
      where: { id_stock },
      include: [
        {
  ...
  const equals = (productA, productB) => {
    if (
      productA.id_stock === productB.id_stock &&
      productA.id_product === productB.id_product
    )
      if (
        productA.unit_price_input === productB.unit_price_input &&
        productA.amount_input === productB.amount_input
      )
        return true
  ...
  const containsAButNotB = (a = [], b = []) =>
    a.filter(itemA => {
      if (!b.find(itemB => equals(itemB, itemA))) return itemA
    })
  ...
  StockInput.creator = async data => {
    try {
      ...
      await sequelize.models.stock_input_products.addProducts(
        data.stock_input_products,
        inputCreate.id_stock
      )
  ...
  StockInput.delete = async id_stock => {
    try {
      ...
      let inputProducts = await sequelize.models.stock_input_products.findAll({
        where: { id_stock },
      })
      ...
      inputProducts.map(async inputProduct => {
        ...
        let stockProduct = await sequelize.models.stock_products.findOne({
          where: { id_product: inputProduct.id_product },
        })
        ...
        sequelize.models.stock_products.update(productUpdate, {
          where: { id_product: stockProduct.id_product },
        })
      })
